(() => {
  document.documentElement.classList.add("game-engine-week-eleven-root");
  ...
  const writeSessionStorage = (key, value) => {
    try {
      ...
      // Identity fields remain usable when session storage is blocked.
    }
  };
  ...
  const removeSessionStorage = (key) => {
    try {
      ...
      // Visible identity fields can still be cleared when storage is blocked.
    }
  };
  ...
  const buildStorageKey = "game-engine-1-week-11-build-v1";
  ...
  if (buildChecklist && buildProgressLabel) {
    ...
    armReset(buildReset, {
      ...
      confirmMessage: "제출 체크 8개를 모두 지우려면 버튼을 한 번 더 누르세요.",
  ...
  const testStorageKey = "game-engine-1-week-11-tests-v1";
  const testIdentityKey = "game-engine-1-week-11-identity-v1";
  ...
  if (testTable && testProgress && testProgressLabel) {
    ...
    exportTests?.addEventListener("click", () => {
      ...
      const header = ["ID", "필수", "조작 조건", "기대 결과", "판정", "관찰 및 수정 기록"];
      ...
      csvRows.push(["수업", "Game Engine I / Week 11 / AI-Assisted Coding and Time Bonus"].map(csvCell).join(","));
  ...
  const timeContractLab = document.querySelector("[data-time-contract-lab]");

  if (timeContractLab) {
    const stateInput = timeContractLab.querySelector("[data-time-state]");
    const currentInput = timeContractLab.querySelector("[data-time-current]");
    const bonusInput = timeContractLab.querySelector("[data-time-bonus]");
    const resultOutput = timeContractLab.querySelector("[data-time-result]");
    const explanation = timeContractLab.querySelector("[data-time-explanation]");
    const resultPanel = timeContractLab.querySelector(".time-contract-result");
    const cap = 30;
  ...
  const navigation = document.querySelector("[data-week-eleven-toc]");
